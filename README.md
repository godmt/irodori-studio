...
1. 上部が`MODEL READY`になるまで待ちます。自動ロードを無効化した場合は右上からモデルを選択します。
2. 左下の「現在のボイス」からボイスライブラリを開きます。
3. Speaker Inversion、参照音声、参照なしのいずれかを設定します。
4. 台本行の再生、または「ここから連続再生」を開始します。
5. 更新アイコンで新しいテイクを作り、番号で採用テイクを選びます。
...
| `workspace/projects/` | 明示的に保存したプロジェクトJSON |
| `workspace/exports/` | 動画・配信用ZIP |
| `workspace/voicevox/profiles.json` | 公開ボイス、固定Style ID、API生成設定 |

`workspace/`、`.studio/`、モデル、個人音声はGit管理されません。ブラウザー側の作業中プロジェクトは`localStorage`にも自動保存されます。

...
export const api = {
  ...
  models: () => request("/api/models").then((response) => response.json()),
  installModel: (modelId) => request("/api/models/install", { method: "POST", body: JSON.stringify({ model_id: modelId }) }).then((response) => response.json()),
  modelInstall: (id) => request(`/api/model-installs/${encodeURIComponent(id)}`).then((response) => response.json()),
